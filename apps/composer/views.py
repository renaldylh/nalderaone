...
def _fetch_feed_events_for_workspace(feeds):
    ...
    headers = {
        ...
        "User-Agent": "Nalderaone RSS Reader/1.0",
    }
    all_events = []
    for feed in feeds:
...
def _validate_rss_url(rss_url):
    ...
    headers = {
        "Accept": "application/rss+xml, application/atom+xml, application/xml, text/xml;q=0.9, */*;q=0.1",
        "User-Agent": "Nalderaone RSS Validator/1.0",
    }
...
def feed_add(request, workspace_id):
    ...
    category = request.POST.get("category", "nalderaone-favorites")
...
def feed_explore(request, workspace_id):
    ...
    workspace = _get_workspace(request, workspace_id)
    category = request.GET.get("category", "nalderaone-favorites")
    return _render_explore(request, workspace, category)
